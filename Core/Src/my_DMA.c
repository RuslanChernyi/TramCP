...
void DMA_SPI1TXInit(void)
{
	...
	RCC->AHB1ENR |= (1U<<22);
	// Disable the stream
	DMA2_Stream3->CR &= ~(1U<<0);	// Disable the stream0
	while((DMA2_Stream3->CR & (1U<<0))) // Check that stream is disabled, if not disable again
	{
		DMA2_Stream3->CR &= ~(1U<<0);	// Disable the stream0
	}
	...
	DMA2_Stream3->M0AR =(uint32_t)dummy_buffer;
	// Configure number of items to be transferred in the DMA_SxNDTR register.
	DMA2_Stream3->NDTR = 1;
	//Select channel for a stream
	DMA2_Stream3->CR |= 0x3<<25;	// Select channel 3 for stream 0 by writing to CHSEL[2:0]
	// Set a peripheral controller in DMA_CR register
	DMA2_Stream3->CR |= (1U<<5);	// Set a PFCTRL bit to enable peripheral flow controller
	// Configure stream priority
	DMA2_Stream3->CR |= (0x2U<<16); // Select high priority for this stream
	// Select direction for a stream
	DMA2_Stream3->CR |= ~(0x3<<6);
	DMA2_Stream3->CR |= (0x1<<6);	// Direction: Memory-to-peripheral. Select by writing into DIR[1:0]
	// Enable memory increment
	DMA2_Stream3->CR |= (1U<<10); // Set MINC
	// Configure memory data size
	DMA2_Stream3->CR &= ~(0x3<<13); // Set memory size to byte(8-bit)
	...
	SPI1->CR2 |= (1U<<1);	// Tx Buffer DMA enable
}
...
void DMA_SPI2TXInit(void)
{
	...
	// Disable the stream
	DMA1_Stream4->CR &= ~(1U<<0);	// Disable the stream3
	while((DMA1_Stream4->CR & (1U<<0))) // Check that stream is disabled, if not disable again
	{
		DMA1_Stream4->CR &= ~(1U<<0);	// Disable the stream0
	}
	// Clear dedicated stream status bits
	...
	// Set memory address in the DMA_SxMA0R register.
	DMA1_Stream4->M0AR =(uint32_t)dummy_buffer;
	// Configure number of items to be transferred in the DMA_SxNDTR register.
	DMA1_Stream4->NDTR = 1;
	//Select channel for a stream
	DMA1_Stream4->CR |= 0x0<<25;	// Select channel 0 for stream 3 by writing to CHSEL[2:0]
	// Set a peripheral controller in DMA_CR register
	DMA1_Stream4->CR |= (1U<<5);	// Set a PFCTRL bit to enable peripheral flow controller
	// Configure stream priority
	DMA1_Stream4->CR |= (0x2U<<16); // Select high priority for this stream
	// Select direction for a stream
	DMA1_Stream4->CR |= ~(0x3<<6);
	DMA1_Stream4->CR = (0x1<<6);	// Direction: Memory-to-peripheral. Select by writing into DIR[1:0]
	// Enable memory increment
	DMA1_Stream4->CR |= (1U<<10); // Set MINC
	// Configure memory data size
	DMA1_Stream4->CR &= ~(0x3<<13); // Set memory size to byte(8-bit)
