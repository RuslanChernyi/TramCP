...
//  SPI1->CR1 |= (1U<<6);
  ...
  SPI2->CR2 |= (1U<<6);
  REG_OSC osc_reg = {0};
  uint8_t big_buffer[752] = {0};
  spican_readBytes(cREGADDR_CiCON, big_buffer, sizeof(big_buffer));
...
//  Configure_CAN();
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  {
//		  uint8_t buffer[6] = {0};
//		  uint16_t writeCommand = (/*cREGADDR_OSC*/ /*cREGADDR_IOCON*/ cREGADDR_CiCON & 0x0FFF) | (0x3 << 12);
